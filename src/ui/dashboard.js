...
export function renderDashboard(container, state, rerender) {
  ...
  if (state.view === 'chat' && state.activeAgent) {
    renderChatView(container, state, rerender);
    return;
  }
  if (state.view === 'edit' && state.editingAgent) {
    renderEditView(container, state, rerender);
    return;
  }
  if (state.view === 'fabrique') {
    renderFabriqueView(container, state, rerender);
    return;
  }
  ...
  if (state.view === 'agents') {
    const btnFabrique = btn('+ Fabrique', 'primary', async () => { state.view = 'fabrique'; rerender(); });
    ...
    const btnExportAll = btn('⬇ Export', '', () => {
      downloadText('nestor-agents.json', exportAgentsJson(state.agents));
    });
...
function renderAgentsList(container, state, rerender) {
  ...
  state.agents.forEach((agent) => {
    ...
    if (agent.preferences && agent.preferences.length > 0) {
      ...
      card.appendChild(topRow); card.appendChild(descEl); card.appendChild(tagsRow); card.appendChild(prefEl);
    } else {
    ...
    const btnEdit = btn('✏️ Éditer', '', () => {
      state.editingAgent = JSON.parse(JSON.stringify(agent)); // clone
      state.view = 'edit';
      rerender();
    });
...
function renderChatView(container, state, rerender) {
  ...
  async function send() {
    ...
    // Injection des préférences dans le system prompt au moment de l'appel
    ...
    try {
      ...
      const reply = choice?.message?.content || '(pas de réponse)';
      state.chatHistory.push({ role:'assistant', content: reply });

      // Détection auto si l'Orchestrateur suggère de créer un agent
...
// ─── Auto-création agent depuis réponse Orchestrateur ─────────────────────────
async function tryAutoCreateAgent(reply, state, rerender) {
  try {
    ...
    if (confirm('🏭 Orchestrateur propose un nouvel agent : "' + newAgent.name + '".\nL'ajouter au registre ?')) {
      ...
      showToast('Agent "' + newAgent.name + '" créé automatiquement.');
      rerender();
    }
  } catch (_) { /* JSON mal formé, on ignore */ }
}

// ─── Vue Fabrique d'agents (création manuelle) ────────────────────────────────
function renderFabriqueView(container, state, rerender) {
  ...
  desc.textContent = 'Décris l\'agent en quelques mots et clique "Générer". La Fabrique créera un agent spécialisé prêt à l\'emploi. Tu pourras ensuite l\'affiner.';
  container.appendChild(desc);

  // Champ brief
  const briefLabel = label('Brief (ex: agent pour suivre mes abonnements)');
  ...
  briefInput.rows = 3;
  briefInput.placeholder = 'Agent pour gérer mes abonnements mensuels...';
  container.append(briefLabel, briefInput);

  // Choix backend
  const backendLabel = label('Backend LLM');
...
function renderEditView(container, state, rerender) {
  ...
  const fields = [
    ...
    { key:'role', label:'Rôle (slug)', type:'text' },
    { key:'description', label:'Description', type:'textarea' },
    { key:'system_prompt', label:'System prompt', type:'textarea', rows:8 },
  ];
  fields.forEach(({ key, label:lbl, type, rows }) => {
    const lEl = label(lbl);
    let input;
    if (type === 'textarea') {
    ...
    input.oninput = () => { agent[key] = input.value; };
    form.append(lEl, input);
  });

  // Backend select
  const bLabel = label('Backend');
  ...
  if (agent.preferences && agent.preferences.length > 0) {
    const pLabel = label('📋 Préférences apprises (' + agent.preferences.length + ')');
...
function renderSettings(container, state, rerender) {
  ...
  noteEl.innerHTML = '🔑 Les clés API sont stockées localement sur cet appareil uniquement.<br>🟢 Puter.js ne nécessite aucune clé.';
  ...
  backends.forEach((b) => {
    ...
    if (b.requiresApiKey && b.envKey) {
      ...
      input.value = lsGet(b.envKey);
      input.onchange = () => { lsSet(b.envKey, input.value.trim()); showToast('Clé ' + b.envKey + ' sauvegardée.'); };
      card.append(lEl, input);
    } else {
